...
import cn.saber.mapper.MessageMapper;
...
import cn.saber.pojo.em.RoleEnum;
...
public class MessageServiceImpl implements MessageService {
    ...
    public Result<Void> systemInfoUsersSave(Message message) {
        ...
        userList.forEach(user -> {
            ...
            if (Objects.equals(RoleEnum.USER.getRole(), user.getUserRole())) {
                ...
                // 发送者用户ID
                saveEntity.setSenderId(senderId);
                // 接收者用户ID
     ...
     */
    @Override
    public Result<Void> clearOneMessage(Message message) {
        messageMapper.updateOne(message.getId(), IsReadEnum.READ_OK.getStatus());
        return ApiResult.success();
    }
}
